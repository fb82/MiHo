...
def to_latex(csv_data, csv_order, renaming_list, header_hold=None, header_bar=None, prev_latex_table=None, add_footer=True, caption_string=None, page_align='landscape', remove_nan_column=False, resize_mode='width'):
    ...
    for i, row in enumerate(bar_csv):
        ...
        if (i>1) and (((i-2)//5)%2!=1): row[0] = '\\rowcolor{gray!15} ' + row[0]
...
def to_latex_prev(csv_data, csv_order, renaming_list, header_hold=None, header_bar=None, prev_latex_table=None, add_footer=True, caption_string=None, page_align='landscape', remove_nan_column=False, resize_mode='width'):
    ...
    header = [
        ...
        '\t\\centering\n',
        resize_what,
        '\t\t\t\\begin{tabular}{L{\\widthof{Key.Net+AffNet+HardNet+NNR++}}C{\widthof{.RANSAC.}}' + ('L{\\MAX}' * (len(header_type)-1)) + '}\n',
    ]
...
if __name__ == '__main__':    
    ...
    pipe_heads = [
          [ '01Key.Net+AffNet+HardNet+NNR', pipe_base.keynetaffnethardnet_module(num_features=8000, upright=True, th=0.99)],
          [                   '01SIFT+NNR', pipe_base.sift_module(num_features=8000, upright=True, th=0.95, rootsift=True)],     
...
###
    ...
    header_hold_full = 'nmmmmmmmmmmmsssssssssss'
    ...
    for ip in range(len(pipe_heads)):
        ...
        latex_table_full = to_latex(fused_csv, fused_csv_order, pipe_renamed, header_hold=header_hold_full, prev_latex_table=latex_table_full, add_footer=((ip + 1) % full_el == 0) or (ip == len(pipe_heads) - 1), caption_string='Full results ' + str(ip // full_el))

        latex_table_full_standalone = to_latex(fused_csv, fused_csv_order, pipe_renamed, header_hold=header_hold_full)
