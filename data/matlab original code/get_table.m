        ...
        for j=1:size(corr_method,1)
            for hom=0:1
                for kk=1:4:size(e,1)                    
                    ...
                    m_table(z,i,j,hom+1,1,ceil(kk/4))=mean(data.err1);
                    m_table(z,i,j,hom+1,2,ceil(kk/4))=mean(data.err2);

                    m_norm_table(z,i,j,hom+1,1,ceil(kk/4))=mean(data.err1/ebase(kk));
                    m_norm_table(z,i,j,hom+1,2,ceil(kk/4))=mean(data.err2/ebase(kk));

                    std_table(z,i,j,hom+1,1,ceil(kk/4))=std(data.err1);
                    std_table(z,i,j,hom+1,2,ceil(kk/4))=std(data.err2);

                    std_norm_table(z,i,j,hom+1,1,ceil(kk/4))=std(data.err1/ebase(kk));
                    std_norm_table(z,i,j,hom+1,2,ceil(kk/4))=std(data.err2/ebase(kk));

                    time_table(z,i,j,hom+1,1,ceil(kk/4))=sum(data.time1)/size(data.mm1,1)*10;
                    time_table(z,i,j,hom+1,2,ceil(kk/4))=sum(data.time2)/size(data.mm2,1)*10;                    
                end
            end
        end
    end
...
m_table=cat(2,m_table,mean(m_table,2));
m_norm_table=cat(2,m_norm_table,mean(m_norm_table,2));
m_norm_table=m_norm_table*100;

std_table=cat(2,std_table,mean(std_table.^2,2).^0.5);
std_norm_table=cat(2,std_norm_table,mean(std_norm_table.^2,2).^0.5);
std_norm_table=std_norm_table*100;

time_table=cat(2,time_table,mean(time_table,2));

%%%

m_table=cat(6,m_table,mean(m_table,6));
m_norm_table=cat(6,m_norm_table,mean(m_norm_table,6));

std_table=cat(6,std_table,mean(std_table.^2,6).^0.5);
std_norm_table=cat(6,std_norm_table,mean(std_norm_table.^2,6).^0.5);

time_table=cat(6,time_table,mean(time_table,6));
...
std_table_=m_table_;
std_norm_table_=m_table_;
time_table_=m_table_;
for z=1:size(method,1)
    widx=z;
    for i=1:length(p)
        ppath=[dpath filesep p(i).name filesep 'working_' method{widx,1}];    
        ...
        for hom=0:1
            for w=0:2
                for kk=1:4:size(e,1)                    
                    ...
                    m_table_(z,i,w+1,hom+1,1,ceil(kk/4))=mean(data.err1);
                    m_table_(z,i,w+1,hom+1,2,ceil(kk/4))=mean(data.err2);

                    m_norm_table_(z,i,w+1,hom+1,1,ceil(kk/4))=mean(data.err1/ebase(kk));
                    m_norm_table_(z,i,w+1,hom+1,2,ceil(kk/4))=mean(data.err2/ebase(kk));

                    std_table_(z,i,w+1,hom+1,1,ceil(kk/4))=std(data.err1);
                    std_table_(z,i,w+1,hom+1,2,ceil(kk/4))=std(data.err2);

                    std_norm_table_(z,i,w+1,hom+1,1,ceil(kk/4))=std(data.err1/ebase(kk));
                    std_norm_table_(z,i,w+1,hom+1,2,ceil(kk/4))=std(data.err2/ebase(kk));

                    time_table_(z,i,w+1,hom+1,1,ceil(kk/4))=sum(data.time1)/size(data.mm1,1)*10;
                    time_table_(z,i,w+1,hom+1,2,ceil(kk/4))=sum(data.time2)/size(data.mm2,1)*10;                    
                end
            end
        end
    end
...
m_table_=cat(2,m_table_,mean(m_table_,2));
m_norm_table_=cat(2,m_norm_table_,mean(m_norm_table_,2));
m_norm_table_=m_norm_table_*100;

std_table_=cat(2,std_table_,mean(std_table_.^2,2).^0.5);
std_norm_table_=cat(2,std_norm_table_,mean(std_norm_table_.^2,2).^0.5);
std_norm_table_=std_norm_table_*100;

time_table_=cat(2,time_table_,mean(time_table_,2));

%%%

m_table_=cat(6,m_table_,mean(m_table_,6));
m_norm_table_=cat(6,m_norm_table_,mean(m_norm_table_,6));

std_table_=cat(6,std_table_,mean(std_table_.^2,6).^0.5);
std_norm_table_=cat(6,std_norm_table_,mean(std_norm_table_.^2,6).^0.5);

time_table_=cat(6,time_table_,mean(time_table_,6));
...
doc_header=[...
    ...
    '\\newcommand*\\Chart[2]{\\rlap{\\textcolor{blue!25}{\\rule[-2.8pt]{\\MAX}{2.4ex}}}\\rlap{{\\textcolor{blue!50}{\\rule[-2.8pt]{#2\\MAX}{2.4ex}}}}#1}\n' ...        
...
out_file=[opath filesep 'report.tex'];
...
for ii=1:length(pl)
    ...
    for zz=1:length(zv)
        ...
        for j=1:size(corr_method,1)
            for hom=1:2
                for ss=1:2
                    ...
                    stable=[stable; squeeze(std_table(z,i,j,hom,ss,:))'];
                    sntable=[sntable; squeeze(std_norm_table(z,i,j,hom,ss,:))'];
                    ttable=[ttable; squeeze(time_table(z,i,j,hom,ss,:))'];                 
                end
            end
        end
        ...
        tolatex(fid,mtable,'%.2f',[c_prefix ' Mean (px)'],[l_prefix '_mean'],1);
        tolatex(fid,mntable,'%.1f',[c_prefix ' Normalized Mean (\\%%)'],[l_prefix '_nmean'],1);
        fprintf(fid,'\t\\newpage\n');   
        tolatex(fid,stable,'%.2f',[c_prefix ' Std (px)'],[l_prefix '_std'],1);       
        tolatex(fid,sntable,'%.1f',[c_prefix ' Normalized Std (\\%%)'],[l_prefix '_nstd'],1);
        fprintf(fid,'\t\\newpage\n');   
        tolatex(fid,ttable,'%.2f',[c_prefix ' Times (s)'],[l_prefix '_time'],1);   
        fprintf(fid,'\t\\newpage\n');        
    end
end
...
for ii=1:length(pl)
    ...
    for zz=1:length(zv)
        ...
        for j=1:3
            for hom=1:2
                for ss=1:2
                    ...
                    stable_=[stable_; squeeze(std_table_(z,i,j,hom,ss,:))'];
                    sntable_=[sntable_; squeeze(std_norm_table_(z,i,j,hom,ss,:))'];
                    ttable_=[ttable_; squeeze(time_table_(z,i,j,hom,ss,:))'];                 
                end
            end
        end
        ...
        tolatex(fid,mtable_,'%.2f',[c_prefix ' Mean (px)'],[l_prefix '_mean'],2);
        tolatex(fid,mntable_,'%.1f',[c_prefix ' Normalized Mean (\\%%)'],[l_prefix '_nmean'],2);
        fprintf(fid,'\t\\newpage\n');   
        tolatex(fid,stable_,'%.2f',[c_prefix ' Std (px)'],[l_prefix '_std'],2);       
        tolatex(fid,sntable_,'%.1f',[c_prefix ' Normalized Std (\\%%)'],[l_prefix '_nstd'],2);
        fprintf(fid,'\t\\newpage\n');   
        tolatex(fid,ttable_,'%.2f',[c_prefix ' Times (s)'],[l_prefix '_time'],2);   
        fprintf(fid,'\t\\newpage\n');        
    end
end
...
function tolatex(fid,table_data,fmt,table_caption,table_label,what)

stable=arrayfun(@(x) num2str(x,fmt), table_data, 'UniformOutput', false);
lval=max(cellfun(@(x) length(x), stable),[],'all');
pct_table=table_data/max(table_data,[],'all');
...
for i=1:size(table_data,1)
    ...
    for j=1:size(table_data,2)
        ...
        aux=[aux '& \\Chart{' oo stable{i,j} '}{' num2str(pct_table(i,j)) '} '];
    end
    ctable=[ctable; {aux}];
end
