import os
import src.base_modules_dev as pipe_base
...
def csv_merger(csv_list, extra_columns=0, csv_aux_list=None):
    ...
    for k, csv_file in enumerate(csv_list):
        aux = [csv_line.split(';') for csv_line in open(csv_file, 'r').read().splitlines() if (not ('nnc' in csv_line)) and (not ('miho' in csv_line))]
        to_fuse = max([idx for idx, el in enumerate([s.startswith('pipe_module') for s in aux[0]]) if el == True]) + 1
...
#   header_clr =  '-gbrtopvtopvgbrtopvtopvgbrtovp'
    ...
    for i, row in enumerate(bar_csv):
        if (i>=1) and not ('downarrow' in bar_csv[i][0]):
            midrule = ''
        else:
            midrule = '\t\t\t\t\\midrule\n'

        if not ('uparrow' in bar_csv[i][0]) and not ('downarrow' in bar_csv[i][0]): row[0] = '\\hspace{0.33em}' + row[0]                 
        if 'uparrow' in bar_csv[i][0]: row[0] = '\\hspace{0.66em}' + '+MAGSAC$_\\uparrow$'                 
        if 'downarrow' in bar_csv[i][0]: row[0] = '\\hspace{0.66em}' + '+MAGSAC$_\\downarrow$'     


        if colrow: row[0] = '\\rowcolor{gray!15} ' + row[0]
        
        if (i>=1) and ('downarrow' in bar_csv[i][0]):
            colrow = not colrow
...
if __name__ == '__main__':    
    ...
    pipe_heads = [
         [      'Keypt2Subpx$^\star$', pipe_base.keypt2subpx_module(num_features=8000, upright=True, what='superpoint', gray=True)],
         [      'Keypt2Subpx', pipe_base.keypt2subpx_module(num_features=8000, upright=True, what='superpoint')],
         [      'SuperPoint$^\star$', pipe_base.lightglue_module(num_features=8000, upright=True, what='superpoint', gray=True)],
         [      'SuperPoint', pipe_base.lightglue_module(num_features=8000, upright=True, what='superpoint')],
         [      'MAst3R$^\star$', pipe_base.mast3r_module(upright=True, gray=True)],
         [      'MASt3R', pipe_base.mast3r_module(upright=True)],
         [      'RoMa$^\star$', pipe_base.roma_module(upright=True, gray=True)],
         [      'RoMa', pipe_base.roma_module(upright=True)],         
        ]
...
#   header_hold = 'nmmm---m---msss---s---shhh---h'
#   header_bar =  '-gbrttttoooogbrttttoooogbrpppp'
    full_el = 8
