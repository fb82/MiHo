...
def show_fig(im1, im2, pt1, pt2, Hdata, Hidx, tosave='miho_torch.pdf', fig_dpi=300,
...
class miho:
    ...
    def all_params():
        ...
        show_clustering_params = {'tosave': 'miho_torch.pdf', 'fig_dpi': 300,
             'colors': ['#FF1F5B', '#00CD6C', '#009ADE', '#AF58BA', '#FFC61E', '#F28522'],
             'markers': ['o','x','8','p','h'], 'bad_marker': 'd', 'bad_color': '#000000',
             'plot_opt': {'markersize': 2, 'markeredgewidth': 0.5,
                          'markerfacecolor': "None", 'alpha': 0.5}}
    ...
    def planar_clustering(self, pt1, pt2):
        ...
        pt1_tensor = torch.tensor(pt1, dtype=torch.float32).to(device)
        pt2_tensor = torch.tensor(pt2, dtype=torch.float32).to(device)

        Hdata = get_avg_hom(pt1_tensor, pt2_tensor, **self.params['get_avg_hom'])
        self.Hs = Hdata

        Hidx = go_assign(Hdata, pt1_tensor, pt2_tensor, **self.params['go_assign'])
        self.Hidx = Hidx.cpu()
