...
def compute_homography_duplex(pt1, pt2, ptm, sidx_par):
    ...
    ## Normalization of pt1_par, pt2_par, and ptm_par (Page 109, 4.4, Multiple View Geometry in Computer Vision)
    ## (i) The points are translated so that their centroid is at the origin.
    ## (ii) The points are then scaled so that the average distance from the origin is equal to 2^0.5
    ## (iii) This transformation is applied to each of the two images independently.
    # 求解pt1_par中最后一维四个点的均值, m = mean(point)
    c1 = torch.mean(pt1_par[:, :2], dim=2)
    c2 = torch.mean(pt2_par[:, :2], dim=2)
    cm = torch.mean(ptm_par[:, :2], dim=2)

    # norm = sum((point-m)^2)^0.5
    ...
    # T12 is a 3d vector for pt1_par and pt2_par, the first index is for parallel batch
    T12 = torch.zeros((l0*2, 3, 3), dtype=torch.float32, device=device)

    # The first half of T12 is the normalization of pt1_par, i.e., the similarity transformation of pt1_par
    T12[:l0, 0, 0] = s1
    T12[:l0, 1, 1] = s1        
    T12[:l0, 2, 2] = 1
    T12[:l0, 0, 2] = -c1[:, 0] * s1
    ...
    # The second half of T12 is the normalization of pt2_par, i.e., the similarity transformation of pt2_par
    T12[l0:, 0, 0] = s2
    T12[l0:, 1, 1] = s2
    T12[l0:, 2, 2] = 1
    T12[l0:, 0, 2] = -c2[:, 0] * s2
    ...
    # Tm is a 3d vector for ptm_par, the first index is for parallel batch
    # Both the first and second half of Tm denote the inverse normalization of ptm_par
    # The inverse of the similarity transformation of ptm_par
    ...
    # A is a 3d vector , the first index is for parallel batch
    A = torch.zeros((l0*2, l1*3, 9), dtype=torch.float32, device=device)


    # The fist half of A is for homography(1->m)
    A[:l0, :l1, 3] = -p1x
    A[:l0, :l1, 4] = -p1y
    A[:l0, :l1, 5] = -1
    ...
    # The second half of A is for homography(2->m)
    A[l0:, :l1, 3] = -p2x
    A[l0:, :l1, 4] = -p2y
    A[l0:, :l1, 5] = -1
...
def sampler4_par(n_par, m):
    ...
    # 重复 min_iter*c_par_sz次: 50*19=950
    n_par = n_par.repeat(m)

    # 每个元素都是在 [0, 1) 范围内的随机值乘以 n_par 相应位置的值 ([2558],[2557],[2556],[2555])
    sidx = (torch.rand((m * nn, 4), device=device) * torch.stack((n_par, n_par-1, n_par-2, n_par-3)).permute(1,0)).type(torch.long)  

    # O(k^2) instead of O(n)
    for k in range(1,4):
        # 前k列进行排序
        sidx[:, 0:k] = torch.sort(sidx[:, 0:k])[0]
        for kk in range(k):
            # sidx[:, k]中的每个元素都会逐一和前k列中的元素比较，如果大于等于，则进行累加操作，这样可以确保每一列都满足递增
            # 1st take a sample in n, next take a sample in n-1, if > of the previous taken index add 1 to the index
...
def ransac_middle(pt1, pt2, dd, th_in=7, th_out=15, max_iter=500, min_iter=50, p=0.9, svd_th=0.05, buffers=5, ssidx=None, par_value=100000):
    ...
    while c < max_iter:
        ...
        # sidx.repeat((1, 1, 4))第三个维度复制了4次，这样得到的张量会变成 (min_iter, c_par_sz, 4 * 4)
        # sidx.repeat_interleave(4, dim=2)这里使用 repeat_interleave 函数将 sidx 张量的第二维度(列维度)中的每个元素复制4次。这样得到的张量会变成 (min_iter, c_par_sz, 4 * 4)
        ...
        # 沿着dd_check的最后一个维度求和, 并与12进行比较，dd_good维度为(min_iter, c_par_sz)
...
def rot_best(pt1, pt2, n=4):
    ...
    # 四种旋转: 0, pi/2, pi, 3pi/2,组成四个维度为2*2的R: [cos(a), -sin(a); sin(a), cos(a)]
...
def get_avg_hom(pt1, pt2, ransac_middle_args={}, min_plane_pts=4, min_pt_gap=4,
    ...
    # 保留每组数据中距离大于15的点对, dd shape: [l, l]
    d1 = dist2(pt1) > th_grid**2
    d2 = dist2(pt2) > th_grid**2
    dd = d1 & d2

    # 转换为齐次坐标, pt1 pt2 shape: [3, l]
...
'''这段代码定义了一个名为 dist2_batch 的函数，它接受一个参数 pt。同样假设代码中有对应的库导入 torch, 这里使用了 PyTorch 库的张量(tensor)操作。

这段代码与之前的 dist2 函数类似，但它似乎用于处理批量的点集，即处理形状为 (batch_size, num_points, 2) 的输入张量 pt,其中 batch_size 表示批处理的样本数量, num_points 表示每个样本中点的数量。

让我们逐步解释这段代码：

pt = pt.type(torch.float32)：将传入的 pt 转换为 torch.float32 类型。

(pt.unsqueeze(-1)[:, :, 0, :] - pt.unsqueeze(1)[:, :, :, 0])**2 + (pt.unsqueeze(-1)[:, :, 1, :] - pt.unsqueeze(1)[:, :, :, 1])**2: 这部分代码计算了批处理的点集中每对点之间的欧几里得距离的平方。

pt.unsqueeze(-1) 在张量的最后一个维度上增加了一个维度，将其形状变为 (batch_size, num_points, 2, 1)。这样做是为了方便进行张量的广播计算。
pt.unsqueeze(1) 在张量的第二个维度上增加了一个维度，将其形状变为 (batch_size, 1, num_points, 2)。同样的原因，为了方便进行张量的广播计算。
(pt.unsqueeze(-1)[:, :, 0, :] - pt.unsqueeze(1)[:, :, :, 0])**2 计算了横坐标差的平方，而 (pt.unsqueeze(-1)[:, :, 1, :] - pt.unsqueeze(1)[:, :, :, 1])**2 计算了纵坐标差的平方。然后将它们相加。
最终得到的是一个形状为 (batch_size, num_points, num_points) 的张量 d, 其中 d[i, j] 表示第 i 个样本中第 j 个点和第 k 个点之间的欧几里得距离的平方。
return d: 将计算得到的欧几里得距离的平方张量 d 返回作为函数的输出。

总结来说，这段代码用于处理批量的点集数据，计算每个样本中每对点之间的欧几里得距离的平方，并将结果作为一个张量返回。'''
def dist2_batch(pt):
...
'''
这段代码定义了一个名为 dist2 的函数，它接受一个参数 pt。假设在代码中有对应的库导入 torch，这里使用了 PyTorch 库的张量（tensor）操作。

让我们逐步解释这段代码：

pt = pt.type(torch.float32)：将传入的 pt 转换为 torch.float32 类型。这行代码假设输入的 pt 是一个张量(tensor),并将其转换为单精度浮点数类型(float32)的张量。

(pt[:, 0, None] - pt[None, :, 0])**2 + (pt[:, 1, None] - pt[None, :, 1])**2: 这是一个计算点之间欧几里得距离的操作。在此代码中，假设 pt 是一个二维张量，其形状为 (N, 2)，其中 N 是点的数量。

pt[:, 0, None] 获取 pt 张量中所有行的第一个列，并增加了一个维度(None/unsqueeze操作),得到形状为 (N, 1) 的张量。
pt[None, :, 0] 获取 pt 张量中所有列的第一个行，并增加了一个维度，得到形状为 (1, N) 的张量。这样做的目的是为了进行广播操作，以便计算所有点两两之间的差。
类似的操作也适用于第二列的索引 1。
(pt[:, 0, None] - pt[None, :, 0])**2 + (pt[:, 1, None] - pt[None, :, 1])**2 计算了每对点之间的横纵坐标差的平方，并将它们相加。这样就得到了一个形状为 (N, N) 的张量 d, 其中 d[i, j] 表示第 i 个点和第 j 个点之间的欧几里得距离的平方。

return d: 将计算得到的欧几里得距离的平方张量 d 返回作为函数的输出。

总之，这段代码用于计算给定点集中每对点之间的欧几里得距离的平方，并将结果作为一个张量返回。'''
def dist2(pt):
    pt = pt.type(torch.float32)
    d = (pt[:, 0, None] - pt[None, :, 0])**2 + (pt[:, 1, None] - pt[None, :, 1])**2
    return d
