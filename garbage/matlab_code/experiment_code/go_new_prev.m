        ...
        for k=1:size(e,1)
            ...
            for j=1:size(corr_method,1)
                for hom=0:1
                    ...
                    if exist(middle_homo_file,'file')~=2                
                        [data.mm1,ttime1]=kpt_improver(im1,im2,to_check_matches,corr_method(j,1),th_sac,1,1,hom,hom_data);
                        [data.mm2,ttime2]=kpt_improver(im1,im2,to_check_matches,corr_method([j j],1),[fix(th_sac/2) th_sac],[0.5 1],1,hom,hom_data);
...
[P1,pidx1]=ransac_similarity(pt2,pt1,th);
[P2,pidx2]=ransac_similarity(pt1,pt2,th);

if sum(pidx1)>sum(pidx2)
    H1=P1;
    H1(1:2,1:2)=H1(1:2,1:2)/sqrt(det(H1(1:2,1:2)));
else
    H2=P2;
    H2(1:2,1:2)=H2(1:2,1:2)/sqrt(det(H2(1:2,1:2)));
end
...
function [matches_new,ttime]=kpt_improver(im1,im2,matches,what,wr,s,ref,hom,hom_data)
