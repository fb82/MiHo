...
def ransac_middle(pt1, pt2, th_in=7, th_out=15, max_iter=10000, min_iter=100, p=0.9, svd_th=0.05, buffers=5):
...
#       test_idx = sum_midx[-1] > sum_midx[:-1]
#       if np.any(test_idx):
#            t = np.argmax(test_idx)

        if sum_midx[-1] > min_th_stats:       
            t = 0
            idxs = list(np.arange(buffers+1))
            qsum = np.zeros(buffers-t)
            for i in range(t, buffers):
                uidx = np.logical_not(np.any(midx[:,idxs[:i]], axis=1)[:, np.newaxis])
                
                ssum = np.sum(uidx & midx[:,idxs[i:]], axis=0)
                ...
                qsum[i-t] = np.max(ssum)
            print(qsum)
            min_th_stats = np.min(qsum)
