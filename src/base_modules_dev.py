...
class lightglue_module:
    ...
    def get_id(self):
        ...
        return (self.what + gray_str + '_lightglue_upright_' + str(self.upright) + '_nfeat_' + str(self.num_features) + '_resize_' + str(self.resize) + plus_str).lower()
    ...
    def run(self, **args):
        if self.gray:
            ...
            gray = cv2.imread(args_['im1'], cv2.IMREAD_GRAYSCALE)
            im_g = cv2.cvtColor(gray,cv2.COLOR_GRAY2RGB)
            cv2.imwrite('tmp1_gray.png', im_g)

            gray = cv2.imread(args_['im2'], cv2.IMREAD_GRAYSCALE)
            im_g = cv2.cvtColor(gray,cv2.COLOR_GRAY2RGB)
            cv2.imwrite('tmp2_gray.png', im_g)

            args_['im1'] = 'tmp1_gray.png'
            args_['im2'] = 'tmp2_gray.png'
        ...
        if self.gray:
            os.remove('tmp1_gray.png')
            os.remove('tmp2_gray.png')
...
class keypt2subpx_module:
    ...
    def get_id(self):
        ...
        return (self.what + gray_str + '_lightglue_keypt2subpx_upright_' + str(self.upright) + '_nfeat_' + str(self.num_features) + '_resize_' + str(self.resize)).lower()
...
class mast3r_module:
    ...
    def get_id(self):
        mast3r_name = 'mast3r_gray' if self.gray else 'mast3r'
        
        return (mast3r_name + '_upright_' + str(self.upright) + '_max_matches_' + str(self.max_matches) + '_resize_' + str(self.resize)).lower()
...
class roma_module:
    ...
    def get_id(self):
         roma_name = 'roma_gray' if self.gray else 'roma' 
         
         return (roma_name + '_outdoor_' + str(self.outdoor) + '_upright_' + str(self.upright) + '_use_tiny_' + str(self.use_tiny) + '_max_keypoints_' + str(self.max_keypoints) + '_cu_res_' + str(self.coarse_resolution) + '_' + str(self.upsample_resolution)).lower()
...
class sift_hardnet_module:
    def __init__(self, **args):
        ...
        with torch.inference_mode():
            ...
            desc = K.feature.HardNet(pretrained=True).to(device)
            self.desc = K.feature.LAFDescriptor(patch_descriptor_module=desc)
    ...
    def run(self, **args):
        ...
        with torch.inference_mode():
            ...
            desc1 = self.desc(im1, kps1).squeeze(0)
        ...
        with torch.inference_mode():
            ...
            desc2 = self.desc(im2, kps2).squeeze(0)

        with torch.inference_mode():
            val, idxs = K.feature.match_smnn(desc1, desc2, self.th)
