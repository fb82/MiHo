...
def ransac_middle_unduplex(pt1, pt2, dd=None, th_grid=15, th_in=7, th_out=15, max_iter=500, min_iter=50, p=0.9, svd_th=0.05, buffers=5, ssidx=None, par_value=100000):
    n = pt1.shape[1]
...
def get_avg_hom_unduplex(pt1, pt2, ransac_middle_args={}, min_plane_pts=4, min_pt_gap=4,
    ...
    while torch.sum(midx) < l - 4:
        ...
        H_, iidx, oidx, ssidx, sidx_ = ransac_middle_unduplex(pt1_, pt2_, dd_, th_grid, ssidx=ssidx, **ransac_middle_args)
        ...
        Hdata.append([torch.matmul(H_, H), idx, sidx_])
...
def cluster_assign_unduplex(Hdata, pt1, pt2, H_pre, median_th=5, err_th=15, **dummy_args):    
    l = len(Hdata)
    n = pt1.shape[0]
    ...
    pt1_ = torch.matmul(H_pre, pt1)
    pt1_ = pt1_ / pt1_[2]

    pt2_ = torch.matmul(torch.inverse(H_pre), pt2)
    pt2_ = pt2_ / pt2_[2]
...
def go_assign_unduplex(Hdata, pt1, pt2, H_pre, method=cluster_assign_unduplex, method_args={}):
    return method(Hdata, pt1, pt2, H_pre, **method_args)
...
class miho:
    ...
    def planar_clustering_unduplex(self, pt1, pt2):
        """run MiHo"""
        self.pt1 = pt1
        self.pt2 = pt2

        Hdata, H_pre = get_avg_hom_unduplex(self.pt1, self.pt2, **self.params['get_avg_hom'])
        self.Hs = Hdata
        self.H_pre = H_pre

        self.Hidx = go_assign_unduplex(Hdata, self.pt1, self.pt2, H_pre, **self.params['go_assign'])
...
class miho_module:
    ...
    def run(self, **args):
        self.miho.planar_clustering_duplex(args['pt1'], args['pt2'])
