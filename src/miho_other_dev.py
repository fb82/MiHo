...
def get_h_distortion(Hs, pt1, pt2):
    l = len(Hs)
...
def cluster_assign_jacobian(Hdata, pt1, pt2, median_th=5, err_th=15, **dummy_args):    
    l = len(Hdata)
    n = pt1.shape[0]
    ...
    err = get_error_unduplex(H, pt1, pt2, sidx_par).permute(1,0) ** 0.5
    
    jerr = get_h_distortion(Hdata, pt1[:2,:].permute((1,0)), pt2[:2,:].permute((1,0))).permute((1,0,2))
