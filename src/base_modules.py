...
class dedodev2_module:
    ...
    def run(self, **args):
        with torch.inference_mode():
            detections1 = self.detector.detect_from_path(args['im1'], num_keypoints = self.num_features)
            kps1 = detections1["keypoints"]
            descs1 = self.descriptor.describe_keypoints_from_path(args['im1'], kps1)["descriptions"]

            detections2 = self.detector.detect_from_path(args['im2'], num_keypoints = self.num_features)
            kps2 = detections2["keypoints"]
            descs2 = self.descriptor.describe_keypoints_from_path(args['im2'], kps1)["descriptions"]
        ...
        kps2 = kps1.squeeze()[idxs[:, 1],:]
        
        pt1, pt2, Hs_laf = refinement_laf(None, None, data1=kps1, data2=kps2, img_patches=False) 

        print("***** dedode v2 kps1 shape: ", kps1.shape)
