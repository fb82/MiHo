...
## How to use?
For installing the code, create a Python virtual environment and after its activation run `pip install -r requirements.txt`. If you are going to run only MOP+MiHo+NCC and not the full benchmark, edit `requirements.txt` as described within its comments. In case of problems installing the `pymulticonsensus` package, you can exclude it from `requirements.txt` and comment the corresponding import and matching module in the code (e.g. lines 18, 97-101 in `run_bench.py`); this will apply to other matching modules too.   
